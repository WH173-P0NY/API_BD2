...
class Employee(db.Model):
    ...
    department_id = db.Column(
        "dzial_id_dzialu", db.Integer, db.ForeignKey("dzialy.id_dzialu")
    )
    position_id = db.Column(
        "stanowisko_id_stanowiska",
        db.Integer,
        db.ForeignKey("stanowiska.id_stanowiska"),
    )

    # Relacja do samodosiebie jako przelozony
    subordinates = db.relationship(
        "Employee", backref=db.backref("superior", remote_side=[id])
    )
    ...
    def to_dict(self):
        return {
            ...
            "date_of_employment": (
                self.date_of_employment.isoformat() if self.date_of_employment else None
            ),
...
class Presence(db.Model):
    ...
    time_of_entry = db.Column("godzina_wejscia", db.DateTime(timezone=False))
    time_of_exit = db.Column("godzina_wyjscia", db.DateTime(timezone=False))
    comment = db.Column("komentarz", db.Text)
    employee_id = db.Column(
        "pracownik_id_pracownika", db.Integer, db.ForeignKey("pracownicy.id_pracownika")
    )
...
class Overtime(db.Model):
    ...
    def to_dict(self):
        return {
            "id": self.id,
            "year_month": (
                self.year_month.strftime("%Y-%m") if self.year_month else None
            ),
...
class Absence(db.Model):
    ...
    absence_type_id = db.Column(
        "typ_nob_id_typu_nob", db.Integer, db.ForeignKey("typy_nob.id_typu_nob")
    )
...
class Allowance(db.Model):
    ...
    type_id = db.Column(
        "typ_dodatku_id_typu_dodatku",
        db.Integer,
        db.ForeignKey("typy_dodatkow.id_typu_dodatku"),
    )

    def to_dict(self):
        return {"id": self.id, "amount": self.amount, "type_id": self.type_id}
...
class AllowanceAssignment(db.Model):
    ...
    payroll_id = db.Column(
        "wynagrodzenie_id_wynagrodzenia",
        db.Integer,
        db.ForeignKey("wynagrodzenia.id_wynagrodzenia"),
        primary_key=True,
    )
    allowance_id = db.Column(
        "dodatki_id_dodatku",
        db.Integer,
        db.ForeignKey("dodatki.id_dodatku"),
        primary_key=True,
    )

    def to_dict(self):
        return {"payroll_id": self.payroll_id, "allowance_id": self.allowance_id}
